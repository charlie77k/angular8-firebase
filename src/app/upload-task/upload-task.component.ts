...
export class UploadTaskComponent implements OnInit {
  ...
  startUpload() {
    const path = `Uploads/${Date.now()}_${this.file.name}`;    // Reference to storage bucket
    ...
    this.snapshot   = this.task.snapshotChanges().pipe(
      ...
      finalize( async() =>  {
        ...
        // this.db.list('files').push({ downloadURL: this.downloadURL, path })        // this.db.collection('files').add( { downloadURL: this.downloadURL, path });
      }),
    );
  }
